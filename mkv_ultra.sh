...
OPTIONS=$(getopt \
	--options hfknt:l:s:x: \
	--longoptions help,force,keep-temp-files,no-replace,temp-dir:,compression-level:,source-dir:,file-name: \
	--name "$0" \
	-- "$@"
)
...
get_files() {
	if [[ -n "$file_name" ]]; then
		...
		find "$source_dir" -type f \( -iname "*.mkv" -o -iname "*.mp4" \) -print0 | sort -zV
	fi
}
